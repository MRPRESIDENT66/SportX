...
/**
 * 启动时确保 spots / challenges 索引按 IK 映射存在。
 *
 * <p>容错处理：ES 不可用（如本地未启动）时仅记录警告、不阻断应用启动，
 * 让不依赖搜索的功能照常工作；待 ES 就绪后可调用各自的 /reindex 重建。
 */
@Slf4j
@Component
...
public class SearchIndexInitializer implements ApplicationRunner {
    ...
    public void run(ApplicationArguments args) {
        try {
            ...
            challengeSearchService.ensureIndex();
        } catch (Exception e) {
            log.warn("Skipped ES index init (Elasticsearch unavailable?): {}", e.getMessage());
        }
    }
}
